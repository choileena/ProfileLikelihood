profilelike.lm <-
function(formula, data, profile.theta, lo.theta=NULL, hi.theta=NULL, length=300, round=2, subset=NULL, weights=NULL, offset=NULL, na.action = stats::na.exclude, ...){
    if(!is.null(subset)){
    ...
    for(i in seq(length)){
        ...
        fit <- stats::lm(y.off ~ -1 + X, na.action = na.action)
